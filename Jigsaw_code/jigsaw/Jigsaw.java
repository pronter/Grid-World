...
public class Jigsaw {
	...
	private void estimateValue(JigsawNode jNode) {
		...
		for (int index = 1; index <= dimension * dimension; index++) {
			if (jNode.getNodesState()[index] != 0) {
				int jNodeRow = (jNode.getNodesState()[index] - 1)  / dimension;
   				...
   				manhattan += Math.abs(jNodeRow - indexRow) + Math.abs(jNodeCol - indexCol);
   				euclidean += Math.sqrt((jNodeRow - indexRow) * (jNodeRow - indexRow) + (jNodeCol - indexCol) * (jNodeCol - indexCol));
			}
		}
        int count = 0;
        for (int index = 1; index < dimension * dimension; index++) {
		...
		jNode.setEstimatedValue(count + s * 3 + manhattan * 2 + euclidean * 1);
	}
}
